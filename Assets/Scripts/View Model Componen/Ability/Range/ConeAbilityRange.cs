...
public class ConeAbilityRange : AbilityRange
{
    ...
    public override List<Tile> GetTilesInRange(Board board)
    {
        ...
        int dir = (unit.dir == Directions.North || unit.dir == Directions.East) ? 1 : -1; //norte o este = 1 sino dir =-1
        int lateral = 1;
        if (unit.dir == Directions.North || unit.dir == Directions.South)
        {
            for (int y = 1; y <= horizontal; ++y) //range
            {
                int min = -(lateral / 2);
                int max = (lateral / 2);
    ...
    {
        return t != null && Mathf.Abs(t.height - unit.tile.height) <= vertical; //nani?
    }
}