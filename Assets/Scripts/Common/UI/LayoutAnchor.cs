...
{
    //2 RT for work (my and parent)
    #region Fields / Properties
    RectTransform myRT;
	RectTransform parentRT;
    #endregion

    //Inicializate and a a warning
	...
	public Vector2 AnchorPosition(TextAnchor myAnchor, TextAnchor parentAnchor, Vector2 offset) // determine where to place our RectTransform
    {
		Vector2 myOffset = GetPosition(myRT, myAnchor);
		Vector2 parentOffset = GetPosition(parentRT, parentAnchor);
		Vector2 anchorCenter = new Vector2(Mathf.Lerp(myRT.anchorMin.x, myRT.anchorMax.x, myRT.pivot.x), Mathf.Lerp(myRT.anchorMin.y, myRT.anchorMax.y, myRT.pivot.y)); //bueno si, alguien quiere mate?
	...
	private Vector2 GetPosition(RectTransform rt, TextAnchor anchor)
	{
		...
		switch (anchor)
		{
            //don't use break, so if enter in one of them, enter in the others (don't like this, but... work...)
